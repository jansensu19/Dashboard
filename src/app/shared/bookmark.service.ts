...
export class BookmarkService implements OnDestroy {
  ...
  updateBookmark(id: string, udpatedFields: Partial<Bookmark>) {
    const bookmark = this.getBookmark(id);
    Object.assign(bookmark, udpatedFields);
  ...
  loadState() {
    try {
      ...
      this.bookmarks.length = 0; // clear the bookmarks array 9 while keeping the reference )
